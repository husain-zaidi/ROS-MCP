...
function executeRosCommand(command: string): string {
  try {
...
async function executeWithTimeout(command: string, timeoutMs: number = 5000): Promise<string> {
...
server.tool(
  ...
  {
    detailed: z.boolean().optional().describe("Show detailed topic information"),
  },
  async (params: { detailed?: boolean }) => {
...
server.tool(
  ...
  {
    topic_name: z.string().describe("The name of the topic to monitor"),
    duration_seconds: z
      .number()
      .positive()
      .max(30)
...
server.tool(
  ...
  {
    topic_name: z.string().describe("The name of the topic"),
  },
  async (params: { topic_name: string }) => {
...
server.tool(
  ...
  {
    topic_name: z.string().describe("The topic name to publish to"),
    message: z.string().describe("The message content in ROS message format"),
    topic_type: z.string().optional().describe("The message type (e.g., std_msgs/String)"),
  },
  async (params: { topic_name: string; message: string; topic_type?: string }) => {
...
server.tool(
  ...
  async (params: { service_name: string; request?: string }) => {
...
server.tool(
  ...
  async (params: { duration_seconds?: number }) => {
    const duration = params.duration_seconds || 5;
    ...
    const staticTf = executeRosCommand(
      `timeout ${duration} ros2 topic echo /tf_static --qos-reliability reliable 2>/dev/null || echo 'No static transforms'`
    );

    const dynamicTf = executeRosCommand(
      `timeout ${duration} ros2 topic echo /tf 2>/dev/null || echo 'No dynamic transforms'`
    );
...
server.tool(
  ...
  {
    node_name: z.string().describe("The name of the node"),
  },
  async (params: { node_name: string }) => {
...
server.tool(
  ...
  {
    node_name: z.string().describe("The name of the node"),
    param_name: z.string().describe("The parameter name"),
    param_value: z.string().describe("The parameter value"),
  },
  async (params: { node_name: string; param_name: string; param_value: string }) => {
    const output = executeRosCommand(
      `ros2 param set ${params.node_name} ${params.param_name} ${params.param_value}`
    );
...
server.tool(
  ...
  async (params: { output_format?: string }) => {
    // Get all nodes
    const nodeList = executeRosCommand("ros2 node list").split("\n").filter((n) => n.trim());

    // Build a graph representation
    let graphData: { [key: string]: { pubs: string[]; subs: string[] } } = {};
...
server.tool(
  ...
  {
    package_name: z.string().describe("The ROS 2 package name"),
    launch_file: z.string().describe("The launch file name (without .launch.py)"),
    args: z
      .string()
      .optional()
      .describe("Additional arguments to pass to the launch file"),
  },
  async (params: { package_name: string; launch_file: string; args?: string }) => {
...
server.tool(
  ...
  {
    package_name: z.string().describe("The ROS 2 package name"),
    node_type: z.string().describe("The node executable name"),
    node_name: z.string().optional().describe("Custom name for the node instance"),
    args: z.string().optional().describe("Additional arguments for the node"),
  },
  async (params: {
    package_name: string;
    node_type: string;
...
server.tool(
  ...
  {
    include_diagnostics: z.boolean().optional().describe("Include diagnostic aggregator info"),
  },
  async (params: { include_diagnostics?: boolean }) => {
    ...
    if (params.include_diagnostics) {
      const diag = executeRosCommand(
        `timeout 5 ros2 topic echo /diagnostics_agg 2>/dev/null || echo 'No diagnostics available'`
      );
      status += `Diagnostics:\n${diag}\n`;
    }
